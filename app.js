(function () {
  ...
  /* 뒤 두 줄은 기대치를 미리 낮추는 자리다. 이 집사는 규칙으로 말을 알아듣기 때문에
     언젠가 반드시 못 알아듣는 날이 온다. 그때 처음 그 얘기를 꺼내면 변명이 되지만,
     첫날 먼저 말해두면 한계가 신입의 성실함으로 읽힌다.
     「못 알아들을 수 있다」가 아니라 「그래도 적는 건 정확하다」에 방점을 둔다. */
  const CAT_FIRST_MEETING = [
    ...
    "집사는 아직 신입이라 주인님 말을 다 알아듣지는 못한다냥. 그래도 받아 적는 건 틀리지 않는다냥.",
    "{owner} 얘기는 뭐든 접수한다냥. 못 알아들은 건 못 알아들은 채로 적어두겠다냥. 지어내진 않는다냥."
  ];
